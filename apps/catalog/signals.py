...
def _incr_version(key: str, initial: int = 1) -> None:
    ...
    """
    # add вернёт True, если ключа не было
    created = cache.add(key, initial)
    try:
        cache.incr(key)
    except Exception:
        # на случай backend без incr — сделаем вручную
...
def product_saved(sender, instance: Product, **kwargs):
    # Деталь продукта (когда появится ProductDetailView)
